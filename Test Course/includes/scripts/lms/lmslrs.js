...
function initCourse(){
	...
	if(courseType.indexOf("hybrid") === -1 && courseType.indexOf("pdf") === -1){
		//Honesty Statements and Bookmarks
		for(var i = 0; i < TOC.length; i++){
			gBookmarks[i] = [];
	...
	//LRS should be initialized for all non-demo courses
...
function lrsInitialize() {
	...
	gUser = new TinCan.Agent({
		...
		"account": {
			...
			"homePage": getHostname() //"homePage": "http://learning.loma.org/" (services.loma.org, ll-content.ccedge.cn, etc)
		}
	});
...
//▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
	...
	// console.log("useChinaCache = " + useChinaCache);
...
function bSetState(){
		...
		callback: function(error, response) {
			if(error !== null){
				if (courseType.indexOf("hioc") !== -1){
					...
					if(response.status >= 400){ //Actually ignore error: 0
	...
	// ------ SCORM, so save to suspend_data
		...
		// }else{
			...
			doLMSCommit();
			console.log("bCheckLRSStatus = " + bCheckLRSStatus);
			if(bCheckLRSStatus === true){
				lrs.dropState(key, xapiSaveStateConfig); // Delete the state object since we aren't using it
				bCheckLRSStatus = false; //set to false so that the extra traffic to the LRS isn't happening when not needed
			}
...
function bGetState(){
	//console.log("bGetStateCalled...");
	
	if (courseType === "hioc"){
		bLRSGetState(true);
		//console.log("bLRSGetState called...");
		return;
	}
	var rawPortalState;
	//AICC or SCORM...
	...
	if(rawPortalState.indexOf("||") !== -1){
		...
		//console.log(`bExamData = ${bExamData}`);
	...
	if(rawPortalState !== ""){
		...
		//console.log(portalStateJSON);
		
		//console.log(JSON.parse(portalStateLMS));
		...
		if(gReturnedState.hasOwnProperty("bAttemptCount")){
			...
			//	doLMSSetValue("cmi.suspend_data", gReturnedState.bAttemptCount);////////////////////////////////////////////////<<<<--------------- This needs to be changed or removed maybe???
			//} 
		}else{ //make sure that this value can never be undefined
			lrsCourseAttempt = 1; 
			bAttemptCount = 1; 
		}
		...
		if(gReturnedState.hasOwnProperty("dateSaved")){dateSaved = gReturnedState.dateSaved;} // Need to do more than this... need to do comparison somewhere?
	...
	//console.log("termsViewed = " + termsViewed);
	...
	xapiGetStateConfig = {
		...
		callback: function(error, response) {
			if(error || response === null){
				if(courseType !== "hioc"){ // only go to the LMS version if this is NOT AICC
			...
			if(goNext === true){
				getLRSEOMState(); // --> proceed to the LRS EOM State, which will call bSetState
			}else{
				getEOMState();
			}
...
document.querySelectorAll(".termslink").onclick = function(){
...
document.querySelectorAll(".privacylink").onclick = function(){
	document.getElementsByClassName("terms_p")[0].innerText = "";
	document.getElementById("closeBtn").style.display = "inline-block";
	document.getElementById("acceptBtn").style.display = "none";
    document.getElementById("terms_cover").style.display = "block";
			...
			if(completionstatus !== "completed" || completionstatus !== "passed" || completionstatus !== "c" || completionstatus !== "p"){
				...
				if (readyForScore === true){
					if(numberOfQuestions){
						...
						console.log("finalWeightedScore = " + finalWeightedScore);
	...
	if(isXAPI){
		...
		if(xapiObject.type === "course"){
			preppedStatement.result = {
				"score": {
					"scaled": (xapiObject.finalScore * .01),
