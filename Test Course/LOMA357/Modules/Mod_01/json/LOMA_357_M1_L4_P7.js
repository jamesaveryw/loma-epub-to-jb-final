Lesson_Data_File([
    {
        "Page_Setup": {
            "Assets_folder": "Mod",
            "Lesson_Header_Color": "#0070b0",
            "Lesson_Title_Color": "#ffffff",
            "Lesson_Title_Text": "",
            "Page_Title": "Risks Associated with Investing",
            "Progress_Bar_Color": "#f2c400",
            "Progress_Bar_Height": "3px",
            "heroSize": "75%",
            "hImage": "",
            "hImage_Alt_Tag_Text": ""
        },
        "JBuilder_Content": [
            {
                "Heading": [
                    {
                        "sngl_Heading": "Risks Associated with Investing"
                    },
                    {
                        "alignment": "center"
                    },
                    {
                        "top_margin": "0px"
                    },
                    {
                        "size": "h2"
                    },
                    {
                        "style": "bold"
                    },
                    {
                        "color": "#000000"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "border_color": ""
                    },
                    {
                        "border_weight": ""
                    },
                    {
                        "border_style": ""
                    },
                    {
                        "border_radius": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Risks Associated with Investing",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "#FBF6D9",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "solid",
                        "border_radius": "0px",
                        "heading": "Leaning Objectives",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": ""
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h1",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "In this chapter, we have discussed investment risk in general terms. Investment risks can also be categorized into two types of risks:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<strong>Systematic risks</strong>. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">systematic risk</span>—also known as a <em>nondiversifiable risk</em> or <em>market risk</em>—has a broad, similar effect on all elements in an economic system and cannot be eliminated through diversification. Within a given market, every investment is exposed to a systematic risk. For instance, the prospect of a shift in a central bank’s monetary policy that leads to a change in market interest rates is a systematic risk to all investments in that market. For a portfolio with investments in multiple countries, an important systematic risk is <em>geopolitical risk</em>, which is the risk that political instability or conflicts between countries adversely affect economic conditions. For example, a war between two neighboring countries is likely to have a negative economic effect on businesses operating in either country."
                            },
                            {
                                "lstText": "<strong>Specific risks</strong>. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">specific risk</span>—also known as a <em>diversifiable risk</em> or <em>unsystematic risk</em>—only affects certain assets or asset classes, and can be substantially reduced by diversification. A specific risk is associated with an individual company or industry, but not an entire market. For instance, the prospect of a new government regulation that affects only the airline industry is a specific risk for airline companies."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "The difference between systematic risk and specific risk hinges on whether the relevant risk can be reduced in an investment portfolio through diversification. The following chart lists examples of systematic risks and specific risks—each of which you will learn about later in this section."
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Systematic Risks"
                            },
                            {
                                "Heading": "Specific Risks"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Asset Bubbles"
                                    },
                                    {
                                        "Data_Info": "Interest-Rate Risk"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Monetary Policy Changes"
                                    },
                                    {
                                        "Data_Info": "Credit Risk"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Inflation"
                                    },
                                    {
                                        "Data_Info": "Real Estate Risk"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Business Cycles"
                                    },
                                    {
                                        "Data_Info": "Liquidity Risk"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Natural Disasters"
                                    },
                                    {
                                        "Data_Info": "Currency Risk"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Systematic Risks",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Systematic risks are hard to eliminate through diversification because these types of risks—many of which have a basis in broad economic conditions—have the potential to affect all investments in the marketplace. Diversification is intended to spread risk across assets, but if all asset choices are subject to the same type of market risk, then diversifying a portfolio does not help an investor avoid that risk."
                    },
                    {
                        "Paragraph_txt": "Investors may not be able to avoid systematic risks, but they can hedge against them. For instance, an investment portfolio holding certain assets—such as bonds—that lose value because of inflation could hedge against that risk by purchasing assets—such as real estate properties—that hold or gain in value during periods of inflation. In this example, bonds and real estate properties are both affected by inflation, but in different ways."
                    },
                    {
                        "Paragraph_txt": "Estimates of expected investment returns incorporate a risk premium for systematic risks but not specific risks. The “price of admission” to participate in a given market is exposure to its systematic risks; thus, investors need risk premiums as incentives to participate in markets. Because well-managed investment portfolios mitigate exposure to specific risks, investors do not require risk premiums to invest in assets that are subject to those risks."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Asset Bubbles",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "One type of systematic risk that can have a dramatic effect on asset prices is the risk of an asset bubble. An <span class=\"keyterm\" tabindex=\"0\" role=\"button\">asset bubble</span>, also called a <em>speculative bubble</em> or <em>price bubble</em>, is a phenomenon in which the market values of an entire asset class rise rapidly, so that prices of assets in the class come to greatly exceed their estimated values. Typically, asset price increases are driven by irrational enthusiasm and outpace increases in the asset’s true value."
                    },
                    {
                        "Paragraph_txt": "In an asset bubble, asset prices rise to unsustainably high levels. Asset bubbles can grow—or “inflate”—for many years before prices reach their peaks. Bubbles can be difficult to spot while they are occurring, but one hallmark is a shift in market sentiment, so that most asset owners are speculators. Toward the peak, asset buyers are spurred by a feeling of missing out on gains and hope to ride the continuous upward momentum of prices."
                    },
                    {
                        "Paragraph_txt": "Eventually, investors and other market participants realize that the asset prices in an asset bubble are due to unfounded optimism, and they sell these assets, causing the bubble to “burst.” After the bubble bursts, asset prices decline quickly, which can cause significant—and painful—losses for the asset owners. Widespread asset bubbles can adversely affect an entire economic system."
                    },
                    {
                        "Paragraph_txt": "Asset bubbles have occurred many times throughout history. Notable bubbles range from the Dutch tulip bubble that burst in 1637 to the U.S. housing bubble that burst around 2007. The insight below describes the burst of the U.S. housing bubble in greater detail."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "0px"
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "<span style=\"font-size:14px;font-weight:bold\">INSIGHT: The Bursting of the U.S. Housing Bubble</span>",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h5",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "In the early 2000s, housing prices in the United States rose rapidly. Some contributing factors to rising house prices were"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "Increased demand from home buyers who qualified for mortgages under lowered underwriting standards"
                            },
                            {
                                "lstText": "A period of low interest rates, which spurred real estate business and investment"
                            },
                            {
                                "lstText": "Increased demand from financial companies for mortgage loans used for structured security origination and trading"
                            },
                            {
                                "lstText": "Government regulations that provided incentives for home ownership"
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Around 2006, a combination of rising interest rates and defaults among high-risk borrowers with low credit ratings started to cause significant issues for the housing market. As mortgage defaults rose and demand for homes fell, housing values declined, resulting in a ripple effect among asset types related to property values. As a result, the market values of properties, mortgages, and related investments—such as mortgage-backed securities and derivatives—also began to decline rapidly, which jeopardized the solvency of financial institutions that held large amounts of these investments."
                    },
                    {
                        "Paragraph_txt": "In 2007, the asset price decreases caused one prominent investment bank to go bankrupt, prompting concerns about the stability of the entire financial system in the United States. During this time, investors generally abandoned risky assets, selling them in large numbers. Demand for safe assets—such as Treasuries—increased rapidly."
                    },
                    {
                        "Paragraph_txt": "In 2008, the financial crisis spread to other parts of the world, particularly Europe. In addition, the United States experienced a significant recession—one of the worst since the 1930s—that adversely affected the global economy. Worldwide, governments and central banks took extraordinary actions to prevent additional economic shocks, and the financial crisis gradually passed."
                    },
                    {
                        "Paragraph_txt": "Since the 2007–2008 financial crisis, several aspects of housing and finance—including mortgage lending standards, credit ratings, uses of capital by financial institutions, and sizes of financial institutions—have received additional scrutiny. As a result, regulators have pushed for increased oversight to attempt to prevent the next financial crisis. In addition, central banks actively search for potential asset bubbles."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "The asset bubble and resulting financial crisis described above resulted from a systematic risk that nearly became a systemic crisis. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">systemic crisis</span> is a series of adverse events occurring in a significant number of financial institutions or markets that has a catastrophic effect. A systemic crisis results in severe impairment or complete failure of the functioning of the financial system."
                    },
                    {
                        "Paragraph_txt": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">systemic risk</span> is a risk that (1) affects securities markets, banking systems, and currencies, and (2) can result in a systemic crisis. Systemic risks are related to systematic risks, but the terms are not interchangeable. A systematic risk can affect an entire market, but a systemic risk threatens to take down an entire interconnected financial system. Thus, only a particular type of systematic risk—such as an asset bubble in real estate prices—can also be a systemic risk. Regulatory entities around the world monitor systemic risks in an effort to prevent a systemic crisis from occurring."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Specific Risks",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "As we mentioned earlier, specific risks can be reduced through diversification. One benefit of portfolio diversification is that the impact of the specific risks associated with a single asset is reduced as more assets are included. Businesses face many kinds of specific risks—such as operational risk and reputational risk—but this section focuses on specific risks associated with investments."
                    },
                    {
                        "Paragraph_txt": "Important categories of specific risks for insurance company investments include interest-rate risk, credit risk, real estate risk, liquidity risk, and currency risk."
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "Interest-Rate Risk",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h4",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Interest-rate risk</span> is the risk that the market value of an investment will change due to a change in nominal market interest rates. In general, for interest-bearing investments—such as bonds and mortgage loans—the market value of an existing investment will"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<strong>Decrease</strong> as market interest rates <strong>increase</strong> "
                            },
                            {
                                "lstText": "<strong>Increase</strong> as market interest rates <strong>decrease</strong> "
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "For instance, in rising interest-rate environments, current issues of bonds typically fall in market value because new issues will be offered at higher coupon rates, which are more attractive to investors. In such cases, an investor can hold existing bonds to maturity, thus avoiding selling them at unfavorable market values. Note that life insurance companies generally purchase bonds and other debt securities with the intention of holding them to maturity."
                    },
                    {
                        "Paragraph_txt": "Interest-rate risk affects virtually all investments. For example, real estate is subject to interest-rate risk because transactions are usually financed with interest-bearing loans. In addition, some stock market sectors—such as those comprising utility companies or financial companies—are more sensitive to interest rates than other sectors."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Reinvestment Risk",
                        "heading_font_size": "h5",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Generally, <span class=\"keyterm\" tabindex=\"0\" role=\"button\">reinvestment risk</span> is the risk that market conditions will be unfavorable when the investor needs to invest the proceeds from a prior investment. For an interest-bearing security, reinvestment risk is the risk that an investor will receive lower interest income after reinvesting the proceeds from an interest-bearing security that has matured. Reinvestment risk also applies to any interest payments that an investor receives while holding an interest-bearing security."
                    },
                    {
                        "Paragraph_txt": "Reinvestment risk increases in a declining interest-rate environment. Insurance companies—and other institutional investors that hold many fixed-income investments—can miss out on investment income as it becomes harder to find bonds of a certain credit quality with coupon rates and yields as high as those for their recently matured bonds. In such a scenario, investors have two choices: (1) increase bond principal amounts to make up for lost income or (2) pursue riskier bonds with higher coupon rates."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Bond Duration",
                        "heading_font_size": "h5",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Bonds have different levels of sensitivity to interest-rate changes, depending on each bond’s coupon rate and term to maturity. Typically, two characteristics associated with greater interest-rate sensitivity are lower coupon rates and longer terms to maturity. In general, bonds with lower coupon rates take longer to generate cash flows that cover the original cost of purchase than bonds with higher coupon rates. Likewise, cash flows from bonds with longer terms to maturity generally take more time to cover the investor’s original cost than those of bonds with shorter terms to maturity. Bonds with greater interest-rate sensitivity also experience greater market value volatility."
                    },
                    {
                        "Paragraph_txt": "Bond sensitivity is measured using <span class=\"keyterm\" tabindex=\"0\" role=\"button\">duration</span>, which is a statistic that measures the interest-rate sensitivity of an interest-bearing security. Duration can be defined as the responsiveness—expressed in years—of a security’s market value to changes in market interest rates. Investors can calculate duration in multiple ways, with different calculations emphasizing cash flows or price changes."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Interpreting Duration</strong> "
                    },
                    {
                        "Paragraph_txt": "A duration statistic of <em>n</em> years indicates that, for a change of 1% in interest rates, the price or market value of a security will change by approximately <em>n</em>% in the opposite direction. Thus, as the value of duration increases, interest-rate risk increases."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "If two bonds are similar but have different durations, then the one with lower duration is less risky. However, the bond with lower duration also has less potential for price appreciation."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "<span style=\"font-size:14px;font-weight:bold\">EXAMPLE</span>",
                        "heading_font_size": "h6",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Zaire Ambrose is an investment analyst who is assessing the interest-rate sensitivity of two bonds. He looks at the bond durations to evaluate the impact of rate changes on market values, as indicated in the following chart:"
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "&nbsp;"
                            },
                            {
                                "Heading": "Bond Duration"
                            },
                            {
                                "Heading": "1% Increase in Market Interest Rates"
                            },
                            {
                                "Heading": "1% Decrease in Market Interest Rates"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Bond A"
                                    },
                                    {
                                        "Data_Info": "4 years"
                                    },
                                    {
                                        "Data_Info": "Market value decrease by 4%"
                                    },
                                    {
                                        "Data_Info": "Market value increase by 4%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Bond B"
                                    },
                                    {
                                        "Data_Info": "6 years"
                                    },
                                    {
                                        "Data_Info": "Market value decrease by 6%"
                                    },
                                    {
                                        "Data_Info": "Market value increase by 6%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Analysis</strong>: Bond B has a longer duration than Bond A and is more sensitive to interest-rate changes. Zaire concludes that the less risky bond is Bond A."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "The following chart summarizes how the length of bond duration signals interest-rate risk and affects bond prices."
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Duration"
                            },
                            {
                                "Heading": "Interest-Rate Risk"
                            },
                            {
                                "Heading": "Price Fluctuation"
                            },
                            {
                                "Heading": "More Likely For Bonds With . . ."
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Longer"
                                    },
                                    {
                                        "Data_Info": "Higher"
                                    },
                                    {
                                        "Data_Info": "More"
                                    },
                                    {
                                        "Data_Info": ". . . Lower coupon rates or longer terms to maturity"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Shorter"
                                    },
                                    {
                                        "Data_Info": "Lower"
                                    },
                                    {
                                        "Data_Info": "Less"
                                    },
                                    {
                                        "Data_Info": ". . . Higher coupon rates or shorter terms to maturity"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Margin Compression",
                        "heading_font_size": "h5",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "In a period of increasing market interest rates, insurance companies can experience margin compression. <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Margin compression</span> refers to the narrowing of an insurer’s interest margin, potentially because of market interest-rate changes. The <span class=\"keyterm\" tabindex=\"0\" role=\"button\">interest margin</span>, also known as the <em>interest-rate margin</em> or the <em>interest spread</em>, is the share of investment earnings that an insurer retains after crediting a share of investment earnings to policyowners."
                    },
                    {
                        "Paragraph_txt": "Typically, when market interest rates are rising, policyowners immediately demand a higher crediting rate on their accounts. The company’s interest-bearing investments, however, continue to earn the older, lower rates that prevailed when the company purchased them. If the company immediately pays the new higher rate on its products, the company’s interest margin will narrow. If the company does not pay the new higher rate on its products, the company will likely lose customers and business from financial professionals. To minimize the risk of margin compression, insurance companies typically try to issue products that limit customer withdrawal rights during a policy’s early years. Watch Video 4.2 for a demonstration of how an insurer’s interest margin can narrow as market interest rates increase."
                    }
                ]
            },
            {
                "Video_Files_New": [
                    {
                        "Video": "https://loma.azureedge.net/videos/LOMA357/4-2video.mp4",
                        "Expert_View": "no",
                        "Expert_View_Alt": "The is for the expert view video series",
                        "top_margin": "20px",
                        "back_color": "#ffffff",
                        "Transcript_Text_font": "jb1",
                        "Transcript_Text_align": "left",
                        "Transcript_Text_color": "#000000",
                        "Person_font": "jb2",
                        "Person_align": "left",
                        "Person_text_color": "#000000",
                        "Person_underline": ""
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<strong>Example of Margin Compression as Market Interest Rates Rise</strong>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<em>[A block labeled Investment Earnings appears.]</em>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "An insurer credits a share of its investment earnings to policyowners.The insurer retains a share, called the interest margin."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<em>[The Investment Earnings lock splits in two. The top half is labeled “Interestmargin.” The bottom half is labeled “Share credited to policy holders.”]</em>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "Over time, market interest rates rise. As they rise, the insurer increasesthe crediting rate for its policyowners."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<em>[A graph shows the Interest Margin narrows over time, while the Sharecredited to policyholders increases over time.]</em>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "Because the crediting rate is increasing faster than the rate the insurerearns on its investments, the insurer’s interest margin narrows."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Margin compression also comes into play during periods of decreasing market interest rates if asset maturities are not properly aligned with the average life of the liabilities. The effects of margin compression due to falling market interest rates are realized slowly over time, as the insurer collects interest and principal payments on older interest-bearing investments, and must reinvest these amounts at lower rates."
                    },
                    {
                        "Paragraph_txt": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Margin expansion</span> refers to an increase in an insurer’s interest margin, potentially because of market interest-rate changes. Margin expansion occurs if the insurer reduces the interest-crediting rates it pays to customers while continuing to earn a higher overall rate of return on its existing invested assets. Margin expansion can also occur when a company experiences an increase in the market value of its existing assets after market interest rates fall."
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "Credit Risk",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h4",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Credit risk</span> is the risk that (1) a borrower defaults on debt obligations by failing to make payments of principal or interest on time or (2) a lender sustains a financial loss based on an adverse change in a borrower’s creditworthiness."
                            },
                            {
                                "Paragraph_text": "Due to the large holdings of debt-related investments in the general account of a typical life insurance company, credit risk exposures are one of the main threats of potential debt investment losses for insurer portfolios."
                            },
                            {
                                "Paragraph_text": "Two categories of credit risk that are relevant to investors are"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<strong>Default risk</strong>. In the context of investing, <span class=\"keyterm\" tabindex=\"0\" role=\"button\">default risk</span> is the risk that an investor will not receive the cash flow payments to which it is entitled because a borrower fails to fulfill its debt obligations. Default risk affects all type of debt securities and also affects corporate equity securities, which lose substantial market value if the issuing company defaults on its debt obligations and becomes insolvent."
                            },
                            {
                                "lstText": "<strong>Downgrade risk</strong>. <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Downgrade risk</span> involves the risk of deterioration in a credit rating assigned to an issuer of a debt security. A credit downgrade does not affect payments of principal or interest to investors. However, market value decreases usually accompany downgrades. Thus, if an investor must sell a downgraded security before maturity, the amount of money raised by the sale will be lower than it would have been prior to the downgrade."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "The main threat of credit risk is that, when an investment goes into default or is downgraded, its market value also decreases. Typically, defaults cause larger market value drops than downgrades."
                    },
                    {
                        "Paragraph_txt": "In the context of investing, <em>credit analysts</em> evaluate the credit quality—or “creditworthiness”—of the issuers of debt securities. As part of their evaluations, credit analysts assess the risk of default or downgrade by issuers."
                    },
                    {
                        "Paragraph_txt": "Typically, sovereign debt is the least risky investment one can make in a given economy, but it still carries risks. <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Sovereign credit risk</span> is the risk that a sovereign government will fail to meet its debt obligations, leading to sovereign default or sovereign debt restructuring. When analysts evaluate a sovereign credit risk, the sovereign government’s ability and willingness to repay borrowed money are both important factors to consider."
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "Real Estate Risk",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h4",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Real estate risk</span> is the potential for loss resulting from a decline in the value of real estate. Exposure to real estate risk can come from directly owning real estate properties or from owning assets—such as mortgages and mortgage-backed securities—whose value depends on real estate property values."
                            },
                            {
                                "Paragraph_text": "Losses on real estate properties can have a variety of causes. For example, low demand for local properties could be the result of new local construction, competitive rates for local rents, or adverse economic conditions. Real estate investors can evaluate real estate risk by monitoring market prices for properties that are similar to the ones they own."
                            },
                            {
                                "Paragraph_text": "For mortgage analysis, investors consider two ratios:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Loan-to-value (LTV) ratio</span>, which is the ratio of a mortgage’s loan balance to the property’s value. Typically, the risk of mortgage default increases as the value of the property decreases."
                            },
                            {
                                "lstText": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Debt service coverage ratio</span>, which is the ratio of a borrower’s monthly earnings—before interest and taxes—to its monthly mortgage payments owed. The higher the ratio, the easier the borrower is able to meet its mortgage loan obligations."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "Liquidity Risk",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h4",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "In the context of investing, <span class=\"keyterm\" tabindex=\"0\" role=\"button\">liquidity risk</span> is the risk that an asset might not be easily sold for its underlying value. Liquidity risk can arise from a lack of buyers, lack of secondary markets, or the structure of the investment—as is often the case in alternative investments."
                            },
                            {
                                "Paragraph_text": "An investment is likely to have limited liquidity if it"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "Is traded privately or in OTC markets instead of on public exchanges"
                            },
                            {
                                "lstText": "Contains restrictions that limit the frequency or timing of ownership transactions"
                            },
                            {
                                "lstText": "Is issued in relatively small outstanding quantities"
                            },
                            {
                                "lstText": "Is subject to regulatory or policy restrictions that limit institutional investor involvement"
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Currency Risk",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Currency risk</span>, also known as <em>foreign exchange risk</em>, is a risk arising from changes in currency exchange rates. When one currency falls in relation to another, the falling currency loses relative value and the rising currency gains relative value. Generally, currency risk can arise from geopolitical factors, fluctuations in the supply and demand of a country’s manufactured goods, inflation, and changes in government leadership."
                    },
                    {
                        "Paragraph_txt": "Manufacturers typically price domestically produced products in the domestic currency; thus, an increase in demand for a country’s products can result in an increase in demand for that country’s currency. Increased demand for a particular country’s currency increases the price of that currency relative to the currencies of other countries."
                    },
                    {
                        "Paragraph_txt": "Investments in foreign markets are subject to currency risk. Investors with exposure to currency risk can hedge their risk by holding foreign exchange derivatives."
                    },
                    {
                        "Paragraph_txt": "The following chart summarizes the methods that investors can use to estimate specific risks."
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Specific Risk"
                            },
                            {
                                "Heading": "How to Estimate the Risk to an Investment"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Interest-Rate Risk"
                                    },
                                    {
                                        "Data_Info": "Measure duration"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Credit Risk"
                                    },
                                    {
                                        "Data_Info": "Analyze credit quality"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Real Estate Risk"
                                    },
                                    {
                                        "Data_Info": "Monitor market prices and mortgage ratios"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Liquidity Risk"
                                    },
                                    {
                                        "Data_Info": "Look for characteristics of limited liquidity"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Currency Risk"
                                    },
                                    {
                                        "Data_Info": "Determine foreign exposure"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "10px",
                        "image": "Lets Review.jpg",
                        "image_alt_text": "",
                        "question": "A systematic risk is a type of investment risk which only affects certain assets or asset classes and can be substantially reduced by diversification.",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "True",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "False",
                                "true_false": true,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "This statement is false. A systematic risk has a broad, similar effecton all elements in an economic system and cannot be avoided through diversification.A <strong>specific risk</strong>—which only affects certain assets or assetclasses—can be substantially reduced by diversification.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "This statement is false. A systematic risk has a broad, similar effecton all elements in an economic system and cannot be avoided through diversification.A <strong>specific risk</strong>—which only affects certain assets or assetclasses—can be substantially reduced by diversification.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "10px",
                        "image": "",
                        "image_alt_text": "",
                        "question": "A particular bond has a duration of 5 years. If market interest rates increase by 1%, then the bond’s market value will (<strong>increase / decrease</strong>) by (<strong>1% / 5%</strong>). Select the correct choice that completes the sentence.",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "Increase; 1%",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Increase; 5%",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Decrease; 1%",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Decrease; 5%",
                                "true_false": true,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "The bond’s duration indicates its sensitivity to interest-rate changes.If rates increase by 1%, then the bond’s market value will <strong>decrease</strong> by <strong>5%</strong>.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "The bond’s duration indicates its sensitivity to interest-rate changes.If rates increase by 1%, then the bond’s market value will <strong>decrease</strong> by <strong>5%</strong>.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "10px",
                        "image": "",
                        "image_alt_text": "",
                        "question": "Match each type of specific risk to the correct definition of the term. For each description, type the correct number choice in the blank.",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "The risk that (1) a borrower defaults on debt obligations by failing to make payments of principal or interest on time or (2) a lender sustains a financial loss based on an adverse change in a borrower’s creditworthiness.",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "The risk posed to the investor from changes in currency exchange rates.",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "The risk that the market value of an investment will change due to a change in nominal market interest rates.",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "The risk that an asset might not be easily sold for its underlying value.",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "The risk representing the potential for loss resulting from a decline in the value of real estate.",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "The risk that market conditions will be unfavorable when the investor needs to invest the proceeds from a prior investment.",
                                "true_false": false,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "The first definition describes credit risk, the second describes currencyrisk, the third describes interest-rate risk, the fourth describes liquidityrisk, the fifth describes real estate risk, and the sixth describes reinvestmentrisk.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "The first definition describes credit risk, the second describes currencyrisk, the third describes interest-rate risk, the fourth describes liquidityrisk, the fifth describes real estate risk, and the sixth describes reinvestmentrisk.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            }
        ]
    }
]);