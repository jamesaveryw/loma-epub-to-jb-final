...
function Flip_Multi_New_2(c_flip_data_stream,c_ccnum)
{
...
if(crdlp === 1)
        {
  c_div_maker = c_div_maker + "<div class='flip_con' style='margin-top:20px;height:auto;background-color:"+flip_card_data[0].main_background_color+"'>";//start  container
        }
...
c_div_maker = c_div_maker + "<div class='col-lg-"+crdn+" col-md-"+crdn+" col-xs-12' style='max-width:120ch; line-height:1.5;height:auto;background-color:"+card_decor_whole_array[cct][0]+"' onclick='flipcard("+cct+")'>";
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=   "+card_decor_whole_array[cct][0]+".

c_div_maker = c_div_maker + "<div class='cardwhole' id='card"+cct+"'></div>";
c_div_maker = c_div_maker + "<div class='frontcard'>";
      ...
      c_div_maker = c_div_maker + "<div class='backcard'>";
...
c_div_maker = c_div_maker + "<p style='margin-top:10px;color:"+card_decor_whole_array[cct][3]+";text-align:"+card_decor_whole_array[cct][5]+"'><"+card_decor_whole_array[cct][6]+">"+card_decor_whole_array[cct][4]+"</"+card_decor_whole_array[cct][6]+"></p>"; 
       ...
       c_div_maker = c_div_maker + "<div aria-label='Select card "+cct+" to flip' type='button' tabindex='0' id='card_"+c_ccnum+"_"+cct+"' style='perspective: 2280px; position: relative;height:350px;'>"; 
           ...
           c_div_maker = c_div_maker + "<div class='front alert alert-info' style='overflow-y:auto; backface-visibility: hidden; transform-style: preserve-3d; position: absolute; z-index: 1; height: 100%; width: 100%; transition: all 0.5s ease-out 0s; transform: rotateY(0deg); background-color:"+card_decor_whole_array[cct][1]+"' onClick='rf_flip("+c_ccnum+","+cct+",0)' onkeydown='rf_flip("+c_ccnum+","+cct+",0)'>"; //c_ccnum
...
/*
function createVariables(cdk){
 // var card_deck = [];

  for (var i = 0; i <= 20; ++i) {
    card_deck[i] = "card_" + cdk;
  }
...
function flipCard(crdn)
{
      console.log("card_" + crdn);
  /*
    switch(crdn)
        {
            case 1:
...
{
  console.log("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
  console.log(comp_number +"_" + crd_number + " | " + event);
  console.log("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
  //$("#card_"+ comp_number +"_" + crd_number).flip();
