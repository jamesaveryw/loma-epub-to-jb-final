...
//console.log("Image_Multi c_data_stream: " + c_data_stream[0].total_images)

            for(var image_collect = 1; image_collect <= c_data_stream[0].total_images; image_collect++)
                {
                    image_count = (image_count + 1); 
                }

/*
            for(var image_collect = 1; image_collect <= 3; image_collect++)
                {

         
                       if(c_data_stream[image_collect].image_file !== "image.jpg" && c_data_stream[image_collect].image_file.length > 4)
                        {
                            console.log("c_data_stream[image_collect].image_file: " + c_data_stream[image_collect].image_file);
                            image_count = (image_count + 1);
                        }
                    ...
                    //console.log("image_collect: " + image_collect);
                }
*/
...
var tagcheck_array = [];
for(var tagcheck = 0; tagcheck <= 3; tagcheck++)
    {
        if(tagcheck === 0)
            {
                tagcheck_array[tagcheck] = "0";
            }

        if(tagcheck >= 1)
            {
                if(typeof(c_data_stream[tagcheck].image_file_tag) === "undefined")
                    {
                        tagcheck_array[tagcheck] = "0";
                    }
                else
                {
                    tagcheck_array[tagcheck] = c_data_stream[tagcheck].image_file_tag;
                }
                
            }
        alt_text_Large[c_ccnum] = tagcheck_array;
    }
...
if(c_data_stream[imgCt].image_file.length > 5)
    ...
    if(c_data_stream[imgCt].image_file_tag.length >= 125)  
        {
           // console.log("c_data_stream[imgCt].image_file_tag: " + c_data_stream[imgCt].image_file_tag);
