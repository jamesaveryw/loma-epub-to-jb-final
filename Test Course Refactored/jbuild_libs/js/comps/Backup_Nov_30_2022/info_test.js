...
function c_Info_Box(c_compData,c_ccnum)
{
	...
	var c_ccData = [[],[]];
	var c_div_maker;
	//alert("Info Box c_ccnum: " + c_ccnum);
	for(var c_count = 0; c_count < c_compData.length; c_count++)
		{
			//alert("c_count: " + c_count);
			if(Object.keys(c_compData[c_count]).length > 1)
				{
					//alert(  (Object.keys(c_compData[c_count])).length  );
					//alert(  String(Object.keys(c_compData[c_count]))  );
					
					c_child_data[c_count] = (Object.keys(c_compData[c_count]))
					
					//alert("c_child_data[c_count]: " + c_child_data[c_count]);
					
					//alert(Object.keys(c_compData[c_count]).length);
					//alert( c_child_data[c_count].Icon );
...
function c_make_info_modal()
	{
//-------<(  Modal Box )>-------										
	//alert("Modal Called c_ccnum: " + c_ccnum);
...
/*
c_div_maker = c_div_maker + "<div class='c_jb_modal-hdr-ftr'><span class='c_jb_close' id='info_btn'><button onclick='javascript:close_Info_modal("+c_ccnum+")'><jb1>" + closeButton + "</jb1></button></span></div>";	//Close											
...
c_Lesson_Builder(c_div_maker);

//Jcc_Component_Builder(c_div_maker);	 //c_ccnum
