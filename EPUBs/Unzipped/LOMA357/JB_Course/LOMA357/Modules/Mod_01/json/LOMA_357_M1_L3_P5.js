Lesson_Data_File([
    {
        "Page_Setup": {
            "Assets_folder": "Mod",
            "Lesson_Header_Color": "#0070b0",
            "Lesson_Title_Color": "#ffffff",
            "Lesson_Title_Text": "",
            "Page_Title": "Types of Investment Funds",
            "Progress_Bar_Color": "#f2c400",
            "Progress_Bar_Height": "3px",
            "heroSize": "75%",
            "hImage": "",
            "hImage_Alt_Tag_Text": ""
        },
        "JBuilder_Content": [
            {
                "Heading_New": [
                    {
                        "sngl_Heading": "Types of Investment Funds"
                    },
                    {
                        "alignment": "center"
                    },
                    {
                        "top_margin": "0px"
                    },
                    {
                        "size": "h2"
                    },
                    {
                        "style": "bold"
                    },
                    {
                        "color": "#000000"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "top_line_color": ""
                    },
                    {
                        "border_background_color": ""
                    },
                    {
                        "bottom_line_color": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "#FBF6D9",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "solid",
                        "border_radius": "0px",
                        "heading": "Learning Objective",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "#015d52",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "An <span class=\"keyterm\" tabindex=\"0\" role=\"button\">investment fund</span>—known in some countries as a <em>collective investment vehicle</em>—is a type of investment vehicle that pools capital to purchase a portfolio of securities. Investors own shares that represent their portion of the portfolio’s returns."
                    },
                    {
                        "Paragraph_txt": "<br />Collectively, investment funds possess large amounts of capital. Figure 3.3 illustrates the total net assets of investment funds registered in the United States (as of 2017)."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Figure 3.3 Net Assets of Investment Funds Registered in the United States</strong>"
                    }
                ]
            },
            {
                "image_w_text": [
                    {
                        "image_name": "3-3figure.jpg",
                        "image_w_text_tag": "The pie chart illustrates the total net assets of investment funds registeredin the United States, as of 2017. The largest slice is open-end mutualfunds, which had $18.7 trillion in assets, or approximately 83.26% of thetotal net assets.<br /><br />The second-largest slice is exchange-traded funds, which had $3.4 trillionin assets, or approximately 15.14% of the total net assets.<br /><br />The third-largest slice is closed-end mutual funds, which had $275 billionin assets, or approximately 1.22% of the total net assets.<br /><br />The smallest slice is unit investment trusts, which had $85 billion inassets, or approximately 0.38% of the total net assets.",
                        "border_style": "",
                        "border_height": "",
                        "border_color": ""
                    },
                    {
                        "image_position": "bottom",
                        "image_aligmnent": "center"
                    },
                    {
                        "image_size": "50%"
                    },
                    {
                        "image_text": "",
                        "image_list": [],
                        "list_position": "bottom",
                        "top_margin": "10px",
                        "Type_of_List": "ol",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1",
                        "list_border_style": "",
                        "list_border_height": "",
                        "list_border_color": "",
                        "image_text_below": ""
                    },
                    {
                        "image_text_color": "#bf0d3f",
                        "image_text_style": "normal",
                        "image_text_variant": "normal",
                        "image_text_weight": "normal",
                        "image_text_top_margin": "0%",
                        "image_text_padding": "0px"
                    },
                    {
                        "image_text_alignmnet": "left",
                        "image_font_size": "jb1"
                    },
                    {
                        "top_margin": "0px",
                        "snippet_top_margin": "10px"
                    },
                    {
                        "background_color": ""
                    },
                    {
                        "image_radius": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<a class=\"pdf_inline_style\" href=\"javascript:jb_src(0)\">Source</a>"
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h1",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "Investment funds have the following characteristics:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<strong>Professional fund management</strong>. Investment professionals include <em>fund managers</em>, who select and manage securities according to a fund’s stated strategy, and <em>fund analysts</em>, whose research and analysis activities support fund managers."
                            },
                            {
                                "lstText": "<strong>Diversification</strong>. By holding multiple securities, each fund spreads out investment risk. Investors that hold multiple funds have the opportunity to further diversify their investments."
                            },
                            {
                                "lstText": "<strong>Economies of scale</strong>. Funds have <span class=\"keyterm\" tabindex=\"0\" role=\"button\">economies of scale</span>, which are unit cost savings that increase as the size of a business operation grows. In this case, <em>units</em> refer to the fund management services that a fund provides. Because funds have some fixed expenses—such as administration costs—an increase in the portfolio’s asset values generally decreases the fund’s cost relative to the services provided."
                            },
                            {
                                "lstText": "<strong>Designated investment styles</strong>. Each fund has a designated investment style that guides the fund’s asset selection and portfolio management strategies. In general, fund managers are required to stick to their fund’s style, although hedge fund managers have more discretion due to the lighter regulatory oversight of hedge funds. We cover fund styles in Chapter 7."
                            },
                            {
                                "lstText": "<strong>Required disclosures</strong>. Regulators require most funds to provide potential and current investors with a <span class=\"keyterm\" tabindex=\"0\" role=\"button\">prospectus</span>, which is a document that describes specific aspects of a fund being offered for sale—such as a fund’s goals, style, managers, expenses, fees, and past performance. The purpose of a prospectus is to provide transparency so that investors can make informed decisions regarding their participation in the fund. Hedge funds are not required to file a prospectus but may provide similar disclosures through an <em>offering memorandum</em>."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Fund investors generally benefit from (1) the expertise of fund managers, (2) access to a broad, diversified investment portfolio, and (3) lower transaction costs—through economies of scale—for a portfolio of securities than if they purchased the securities on their own."
                    },
                    {
                        "Paragraph_txt": "<br />Investment fund types vary in their expense and fee structures. In addition, ownership in each type of fund has different tax implications for investors."
                    },
                    {
                        "Paragraph_txt": "<br />In the previous section, you read about hedge funds. Other types of investment funds include mutual funds, unit investment trusts, and exchange-traded funds."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Mutual Funds",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "#015d52",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">mutual fund</span> is an investment vehicle that pools the funds of investors and uses the funds to buy a diversified portfolio of financial securities. The value of each mutual fund share is determined by calculating the <span class=\"keyterm\" tabindex=\"0\" role=\"button\">net asset value (NAV)</span>, which is the total value of the mutual fund assets—after expenses—divided by the total number of <em>fund shares outstanding</em>, which represents the number of fund shares that shareholders currently own. The NAV is calculated each day after securities markets close."
                    },
                    {
                        "Paragraph_txt": "<br />Before conducting business in the United States, mutual funds must register with the <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Securities and Exchange Commission (SEC)</span>, which is a federal agency responsible for administering and enforcing federal securities laws and regulating the securities industry, as well as for executing policies that affect securities markets. For the purpose of federal securities regulation, mutual funds are legally structured as <span class=\"keyterm\" tabindex=\"0\" role=\"button\">investment companies</span>, which are entities that issue securities and engage primarily in investing, reinvesting, and trading in securities. As such, mutual funds are registered with the SEC as investment companies."
                    },
                    {
                        "Paragraph_txt": "<br />The investment management firm that registers a particular mutual fund with the SEC is known as the <em>fund sponsor</em>. To avoid confusion between the terms “fund sponsor” and “investment company,” this text only uses the term “investment company” when referring to the technical classification under U.S. securities laws."
                    },
                    {
                        "Paragraph_txt": "<br />Two types of mutual funds are open-end mutual funds and closed-end mutual funds."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Open-End Mutual Funds",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Most mutual funds are open-end mutual funds. An <span class=\"keyterm\" tabindex=\"0\" role=\"button\">open-end mutual fund</span> establishes a portfolio of securities with a flexible number of outstanding shares and trades shares directly with investors."
                    },
                    {
                        "Paragraph_txt": "<br />When an investor invests in the fund, the fund issues more shares—thus increasing the total number of outstanding shares. When an investor <em>redeems</em> shares, the fund buys back the investor’s shares—thus decreasing the total number of outstanding shares."
                    },
                    {
                        "Paragraph_txt": "<br />Purchases and redemptions of open-end mutual fund shares are executed once a day, after the market closes. When an investor submits a purchase or redemption order, the price she receives per share is equal to the NAV that was calculated at the end of the day when the order is received. Because investors submit their orders before the NAV is calculated, they do not know the exact price they will receive on the shares they buy or redeem."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Closed-End Mutual Funds",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">closed-end mutual fund</span> establishes a portfolio of securities with a fixed number of outstanding shares that are traded on securities exchanges or in the OTC market. The fund does not issue new shares and does not redeem outstanding shares."
                    },
                    {
                        "Paragraph_txt": "<br />Investors can trade shares in a closed-end mutual fund at any time while the market is open. Because closed-end mutual fund shares are traded in the marketplace, share prices are determined by market forces instead of by NAV calculations. Therefore, a fund’s shares can trade at a (1) <em>discount</em> when the current market price is below its NAV, or (2) <em>premium</em> when the current market price is above its NAV."
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "<strong>Unit Investment Trusts</strong>",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "#015d52",
                        "heading_size": "h3",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">unit investment trust (UIT)</span> is a portfolio of securities assembled for a specific investment purpose and held for a designated amount of time. The shares of a UIT are called <em>units</em>. Like mutual funds, UITs are registered with the SEC by a fund sponsor."
                            },
                            {
                                "Paragraph_text": "UITs share characteristics of open-end and closed-end mutual funds:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "Like open-end mutual fund investors, UIT investors can redeem units for cash, with the price of a unit equal to the UIT’s NAV."
                            },
                            {
                                "lstText": "Like a closed-end mutual fund, a UIT issues a fixed number of units; UIT units are issued on the fund’s <em>creation date</em>."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Unlike mutual funds, a UIT has a fixed investment portfolio. The portfolio’s securities holdings are determined prior to the creation date and usually do not change for the life of the UIT. UITs typically focus on bond holdings or stock holdings."
                    },
                    {
                        "Paragraph_txt": "<br />At the time of creation, a UIT establishes a termination date. Upon termination, any securities in the portfolio are sold and the proceeds are distributed to investors."
                    },
                    {
                        "Paragraph_txt": "<br />Based on the<em> Investment Company Act of 1940</em>—which you will learn about in Chapter 10—the SEC considers UITs, like open-end and closed-end mutual funds, to be a form of an investment company. In addition, the SEC typically classifies insurance company separate accounts as UITs because of their legal structure (the full explanation is beyond the scope of this text). Therefore, most separate accounts are registered with the SEC as UITs to comply with the Investment Company Act."
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "<strong>Exchange-Traded Funds</strong>",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "#015d52",
                        "heading_size": "h3",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "An <span class=\"keyterm\" tabindex=\"0\" role=\"button\">exchange-traded fund (ETF)</span> is a security that usually tracks a market index and trades continuously during the trading day on an exchange. For example, some popular ETFs track a stock market index called the <em>Standard and Poor’s 500 Index</em>—also known as the <em>S&amp;P 500 Index</em>—which consists of 500 of the largest and most widely held companies in the United States."
                            },
                            {
                                "Paragraph_text": "Initially, ETFs only tracked basic market indexes. Over time, ETFs grew to track a larger number of indexes. In recent years, ETF offerings have expanded beyond indexes. For instance, some ETFs track groups of securities selected by investment professionals."
                            },
                            {
                                "Paragraph_text": "ETFs share some characteristics with closed-end mutual funds. For example, ETFs trade continuously while markets are open, thus serving as a highly liquid investment vehicle. In addition, an ETF can also trade at a premium or discount to the fund’s NAV."
                            },
                            {
                                "Paragraph_text": "Closed-end funds and ETFs possess some notable differences:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "Closed-end funds have a fixed number of outstanding shares, while ETFs do not have a fixed number of shares. Through a process that is beyond the scope of this text, ETFs have the ability to create and redeem shares as needed."
                            },
                            {
                                "lstText": "Usually, fund managers choose the securities in closed-end funds, while many ETFs are designed to track a market index. In fact, ETFs that track market indexes are judged on how closely they replicate the index’s performance."
                            },
                            {
                                "lstText": "Closed-end mutual funds are more likely to trade at large premiums or discounts to their NAVs, while ETFs generally trade at prices close to their NAVs."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "In general, ETFs have lower expenses and fees than mutual funds holding a similar group of securities. Furthermore, ETFs have the potential for more favorable tax treatment of capital gains."
                    },
                    {
                        "Paragraph_txt": "<br />As more ETF variations have come onto the market, the popularity of ETFs has grown significantly. Many institutional investors—including life insurance companies—hold ETFs in their portfolios."
                    },
                    {
                        "Paragraph_txt": "<br />The following chart compares the characteristics of each type of investment vehicle mentioned in Chapters 2 and 3. Within each type of vehicle, individual investments may diverge from the general characteristics. For example, the stock of a company with high growth potential is going to score higher on price appreciation potential than the stock of a company with low growth potential."
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Investment Vehicle Type"
                            },
                            {
                                "Heading": "Income Generation"
                            },
                            {
                                "Heading": "Price Appreciation Potential"
                            },
                            {
                                "Heading": "Liquidity"
                            },
                            {
                                "Heading": "Possible Risk Exposure"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Cash and Cash Equivalents"
                                    },
                                    {
                                        "Data_Info": "Little to none"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    },
                                    {
                                        "Data_Info": "Very high"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Bonds"
                                    },
                                    {
                                        "Data_Info": "Steady"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    },
                                    {
                                        "Data_Info": "Moderate, but usually lower for munis"
                                    },
                                    {
                                        "Data_Info": "Low to moderate, depending on credit quality and the type of bond"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Stocks"
                                    },
                                    {
                                        "Data_Info": "Steady, if the stock pays a dividend"
                                    },
                                    {
                                        "Data_Info": "High for common stocks; moderate for preferred stocks"
                                    },
                                    {
                                        "Data_Info": "High if traded on an exchange; moderate if traded OTC"
                                    },
                                    {
                                        "Data_Info": "Moderate to high"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Real Estate Properties"
                                    },
                                    {
                                        "Data_Info": "Steady"
                                    },
                                    {
                                        "Data_Info": "High"
                                    },
                                    {
                                        "Data_Info": "Very low"
                                    },
                                    {
                                        "Data_Info": "Moderate, depending on property location and ownership structure"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Mortgage Loans"
                                    },
                                    {
                                        "Data_Info": "Steady"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    },
                                    {
                                        "Data_Info": "Low, depending on whether the loan is recourse or nonrecourse"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Real Estate Investment Trusts (REITs)"
                                    },
                                    {
                                        "Data_Info": "High"
                                    },
                                    {
                                        "Data_Info": "Moderate"
                                    },
                                    {
                                        "Data_Info": "Moderate to high, depending on if they are public or private"
                                    },
                                    {
                                        "Data_Info": "Moderate"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Structured Securities"
                                    },
                                    {
                                        "Data_Info": "Steady, depending on the tranche"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    },
                                    {
                                        "Data_Info": "Low to moderate, depending on the loans pooled and the tranche"
                                    },
                                    {
                                        "Data_Info": "Lower for senior tranches and higher for subordinated tranches"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Derivatives"
                                    },
                                    {
                                        "Data_Info": "None"
                                    },
                                    {
                                        "Data_Info": "Low"
                                    },
                                    {
                                        "Data_Info": "High if traded on an exchange; moderate if traded OTC"
                                    },
                                    {
                                        "Data_Info": "High"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Alternative Investments"
                                    },
                                    {
                                        "Data_Info": "Depends on the investment, but steady for income-generating investments"
                                    },
                                    {
                                        "Data_Info": "High"
                                    },
                                    {
                                        "Data_Info": "Very low"
                                    },
                                    {
                                        "Data_Info": "High"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Mutual Funds"
                                    },
                                    {
                                        "Data_Info": "Depends on fund style, but high for income-generating styles"
                                    },
                                    {
                                        "Data_Info": "Depends on fund style"
                                    },
                                    {
                                        "Data_Info": "High"
                                    },
                                    {
                                        "Data_Info": "Lower because of diversification, but depends on fund style"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Unit Investment Trusts (UITs)"
                                    },
                                    {
                                        "Data_Info": "Depends on fund style, but high for income-generating styles"
                                    },
                                    {
                                        "Data_Info": "Depends on fund style"
                                    },
                                    {
                                        "Data_Info": "High"
                                    },
                                    {
                                        "Data_Info": "Lower because of diversification, but depends on fund style"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Exchange-Traded Funds (ETFs)"
                                    },
                                    {
                                        "Data_Info": "Depends on fund style, but high for income-generating styles"
                                    },
                                    {
                                        "Data_Info": "Depends on fund style"
                                    },
                                    {
                                        "Data_Info": "High"
                                    },
                                    {
                                        "Data_Info": "Lower because of diversification, but depends on fund style"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "30px",
                        "image": "Lets Review.jpg",
                        "image_alt_text": "",
                        "question": "Which type of mutual fund has a fixed number of outstanding shares and has trades of its shares executed throughout the day (while the market is open)?",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "Open-end mutual fund",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Closed-end mutual fund",
                                "true_false": true,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "In contrast, an open-end mutual fund has a flexible number of outstandingshares and has trades of its shares executed once a day, after the marketcloses.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "In contrast, an open-end mutual fund has a flexible number of outstandingshares and has trades of its shares executed once a day, after the marketcloses.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Inline_Source_File": [
                    {
                        "Source_Info": "Data from Investment Company Institute, <em>2018 Investment Company Fact Book: A Review of Trends and Activities in the Investment Company Industry</em>, 58th edition (Washington, DC: Investment Company Institute, 2018), http://www.icifactbook.org (19 March 2019). Used with permission."
                    }
                ]
            }
        ]
    }
]);