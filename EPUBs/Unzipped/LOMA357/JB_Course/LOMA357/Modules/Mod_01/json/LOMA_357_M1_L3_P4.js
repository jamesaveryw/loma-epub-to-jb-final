Lesson_Data_File([
    {
        "Page_Setup": {
            "Assets_folder": "Mod",
            "Lesson_Header_Color": "#0070b0",
            "Lesson_Title_Color": "#ffffff",
            "Lesson_Title_Text": "",
            "Page_Title": "Alternative Investments",
            "Progress_Bar_Color": "#f2c400",
            "Progress_Bar_Height": "3px",
            "heroSize": "75%",
            "hImage": "",
            "hImage_Alt_Tag_Text": ""
        },
        "JBuilder_Content": [
            {
                "Heading_New": [
                    {
                        "sngl_Heading": "Alternative Investments"
                    },
                    {
                        "alignment": "center"
                    },
                    {
                        "top_margin": "0px"
                    },
                    {
                        "size": "h2"
                    },
                    {
                        "style": "bold"
                    },
                    {
                        "color": "#000000"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "top_line_color": ""
                    },
                    {
                        "border_background_color": ""
                    },
                    {
                        "bottom_line_color": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "#FBF6D9",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "solid",
                        "border_radius": "0px",
                        "heading": "Learning Objective",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "#015d52",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Cash, bonds, and stocks are universally considered <em>traditional investments</em>. Therefore, by definition, an <span class=\"keyterm\" tabindex=\"0\" role=\"button\">alternative investment</span> is any investment that is not considered a traditional investment. However, the term is subjective, because some investors consider real estate, REITs, and derivatives to be alternative investments, while others consider some or all to be traditional investments. (This text does not categorize real estate, REITs, structured securities, or derivatives as alternative investments.)"
                    },
                    {
                        "Paragraph_txt": "<br />Alternative investments can be tangible assets—such as commodities, art, wine, jewelry, coins, baseball cards, and stamps. However, most institutional investors think of hedge funds and private equity funds as the primary types of alternative investments. We explore these in the next sections."
                    },
                    {
                        "Paragraph_txt": "<br />Institutional investors include alternative investments in their portfolios to benefit from these advantages:"
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Advantages of Alternative Investments"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Higher potential returns than they can expect from traditional investments"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Unique risk and return characteristics, which offer increased opportunities for diversification"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Access to strategies or securities not available elsewhere"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "However, many alternative investments have potential disadvantages:"
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Disadvantages of Alternative Investments"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Less liquidity than traditional investments"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Certain restrictions that limit the ability to liquidate the investment, such as multiyear commitments"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Limited information, which makes current market values hard to determine"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Demanding requirements for participation, such as a high minimum for the minimum <span class=\"keyterm\" tabindex=\"0\" role=\"button\">assets under management (AUM)</span>—the amount of money that an advisor or company manages on behalf of a client"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Complexity, requiring the investor to perform significant <span class=\"keyterm\" tabindex=\"0\" role=\"button\">due diligence</span>, which, in the context of investing, is the careful and thorough investigation of a potential investment"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Lack of readily accessible data related to historical investment performance"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Fewer disclosure requirements than traditional investments"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "High management fees and transaction costs"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Three types of alternative investments that are important to life insurers and other institutional investors are hedge funds, private equity funds, and infrastructure investments."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Hedge Funds",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "#015d52",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">hedge fund</span> is an alternative investment vehicle that (1) is often structured as a limited partnership, (2) pools capital from a limited number of sophisticated investors, and (3) is virtually unlimited in the investment strategies it can pursue. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">limited partnership</span> is a business organization that consists of (1) one or more general partners who manage and control a business and are legally liable for all the debts and obligations of the business, and (2) one or more limited partners whose main function is to provide investment capital."
                    },
                    {
                        "Paragraph_txt": "<br />Typically, hedge funds are available only to wealthy individuals and institutional investors and require large initial minimum investments. In addition, hedge funds usually restrict the investor’s ability to get out of the fund, particularly during the period immediately after the investor purchases shares."
                    },
                    {
                        "Paragraph_txt": "<br />Hedge fund participation is limited to sophisticated investors that need less regulatory protection than other investors. Therefore, hedge funds generally face less regulatory oversight and fewer disclosure requirements than other types of investment vehicles."
                    },
                    {
                        "Paragraph_txt": "<br />Because they have fewer regulatory restrictions, hedge funds have the flexibility to engage in a wide range of investment strategies. Most hedge funds focus on a particular fund style that takes advantage of the specialized expertise of their investment professionals."
                    },
                    {
                        "Paragraph_txt": "<br />Hedge funds started out as a means to hedge the risk from holding stocks, hence the fund type’s name. Over time, hedge funds increased their use of leverage to amplify returns. Now hedge funds can use derivatives and leverage to aggressively pursue investment gains and even speculate in markets."
                    },
                    {
                        "Paragraph_txt": "<br />Hedge funds can deliver exceptional returns relative to other types of investments. However, their concentrated bets can expose investors to significant losses. Because concentrated portfolios are less diversified—a concept we explain in Chapter 8—one investment loss could have a large effect on a concentrated hedge fund’s overall return."
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "<strong>Private Equity Funds</strong>",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "#015d52",
                        "heading_size": "h3",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">private equity fund</span> is an alternative investment vehicle that pools capital from sophisticated investors to invest in private companies. Like hedge funds, private equity funds are typically structured as limited partnerships and are available only to wealthy individuals and institutional investors."
                            },
                            {
                                "Paragraph_text": "Most private equity funds require a significant initial investment. Furthermore, investments in private equity funds are illiquid, so investors must be prepared to hold their investments for long periods—such as 10 years."
                            },
                            {
                                "Paragraph_text": "Types of private equity funds include"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<strong>Venture capital funds</strong>. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">venture capital fund</span> provides capital and management expertise to target companies, such as start-up companies with high growth potential. A venture capital fund typically takes an active role in the growth of the target company by providing advice to management and setting goals for the company to achieve."
                            },
                            {
                                "lstText": "<strong>Leveraged buyout funds</strong>. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">leveraged buyout fund</span>, also known as an <em>LBO fund</em>, uses debt financing—such as leveraged loans—to purchase shares of target companies. LBO funds buy the outstanding equity of a target company and use the company’s cash flows to make loan payments on outstanding debt until the debt is repaid."
                            },
                            {
                                "lstText": "<strong>Mezzanine debt funds</strong>. <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Mezzanine debt</span> is a type of subordinated debt that also gives the lender an equity stake in the borrowing company. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">mezzanine debt fund</span> provides mezzanine debt financing to established, profitable companies seeking funds for expansions, acquisitions, or other growth opportunities."
                            },
                            {
                                "lstText": "<strong>Distressed debt funds</strong>. A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">distressed debt fund</span>, also known as a <em>vulture fund</em>, invests in bonds and other debt securities issued by target companies in financial distress. Distressed debt funds search for securities that are undervalued because of a high risk of default, but possess the potential for significant future returns if default is avoided."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Infrastructure Investments",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "#015d52",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Infrastructure</span> refers to the basic facilities and systems that provide the services necessary for societies and economies to function. In other words, infrastructure comprises the physical structures that support cities, states or provinces, and countries. Examples of such structures include roads, bridges, tunnels, sewers, water supply facilities, cables, telephone wires, and electrical grids."
                    },
                    {
                        "Paragraph_txt": "<br />Traditionally, direct investment in infrastructure is done by governments and interested businesses. For example, an energy company may build a pipeline to expand operational capacity. The pipeline benefits the company by enabling it to generate more revenue. The country where the pipeline is built also benefits from enhancement of its infrastructure."
                    },
                    {
                        "Paragraph_txt": "<br />Many institutional investors already have indirect investments in infrastructure through municipal bonds issued by governments and corporate bonds issued by companies in certain industries. However, institutional investors can directly invest in infrastructure by providing capital for infrastructure projects."
                    },
                    {
                        "Paragraph_txt": "<br />As an investment, infrastructure can provide investors with diversification benefits because the risk and return characteristics are unlike many traditional asset classes. In addition, the cash flows generated by infrastructure are attractive to investors such as life insurers. Life insurers want to hold long-term assets to match cash flows with their long-term liabilities. Many infrastructure projects are long-term and have the potential to “provide stable and secure cash flows for life insurance companies, whose liability structure affords them more flexibility in investing in a relatively illiquid asset.”<span class=\"ens\" id=\"loma357-ch3-span-70\"> <span class=\"endnote hidden\" data-note=\"loma357-ch3-en1\" epub:type=\"noteref\" id=\"loma357-ch3-span-71\" role=\"doc-noteref\">1</span> </span> "
                    },
                    {
                        "Paragraph_txt": "<br />Like other alternative investments, infrastructure investments are illiquid. Furthermore, investors may have difficulty (1) finding information for proper due diligence and (2) determining asset values for infrastructure assets."
                    },
                    {
                        "Paragraph_txt": "<br />Another aspect for institutional investors to consider is the infrastructure project cycle. Investors have to commit capital at the beginning to initiate the project, but builders need to complete development before investors can start receiving repayment in the form of incoming cash flows."
                    },
                    {
                        "Paragraph_txt": "<br />The following chart summarizes the investment strategies of the alternative investments covered in this chapter."
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Alternative Investment Type"
                            },
                            {
                                "Heading": "Investment Strategy"
                            }
                        ],
                        "Row_Heading_Color": "#0069ad",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Hedge Fund"
                                    },
                                    {
                                        "Data_Info": "Virtually unlimited; a fund typically focuses on an investment style that utilizes its specialized expertise"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Private Equity Fund"
                                    },
                                    {
                                        "Data_Info": "Invests in private companies; particular type of target company depends on the type of private equity fund"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Infrastructure Investment"
                                    },
                                    {
                                        "Data_Info": "Investor directly invests by providing capital for an infrastructure project"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 105, 173, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "30px",
                        "image": "Lets Review.jpg",
                        "image_alt_text": "",
                        "question": "Institutional investors include alternative investments in their portfolios to benefit from certain advantages. Which of the following is a potential <em>disadvantage</em> of alternative investments?",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "Higher potential returns than traditional investments",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Less liquidity than traditional investments",
                                "true_false": true,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Access to strategies or securities not available elsewhere",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Unique risk and return characteristics",
                                "true_false": false,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "In general, alternative investments are less liquid than traditional investments.Furthermore, an alternative investment may contain restrictions that limitan institutional investor’s ability to access its invested capital fora specified period of time.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "In general, alternative investments are less liquid than traditional investments.Furthermore, an alternative investment may contain restrictions that limitan institutional investor’s ability to access its invested capital fora specified period of time.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "#FBF6D9",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "0px",
                        "image": "",
                        "image_alt_text": "",
                        "question": "Which type of private equity fund uses debt financing to purchase shares of a target company and uses the target company’s cash flows to make loan payments on outstanding debt until the debt is repaid?",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "Distressed debt fund",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Venture capital fund",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Mezzanine debt fund",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Leveraged buyout fund",
                                "true_false": true,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "A leveraged buyout fund uses debt financing—such as leveraged loans—topurchase shares of a target company. Then, the fund uses the target company’scash flows to make loan payments on the debt used to finance the purchase.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "A leveraged buyout fund uses debt financing—such as leveraged loans—topurchase shares of a target company. Then, the fund uses the target company’scash flows to make loan payments on the debt used to finance the purchase.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            }
        ]
    }
]);