...
function checkIbooks() {
	...
	document.body.addEventListener('click', kbInteractiveFix, true);
...
function kbInteractiveFix(e) {
	if (e.target.classList.contains('wdgtIcon') || e.target.classList.contains('widget')) {
		var widgetReady = setInterval(function () {
			var widgetBox = document.getElementById('kInteractiveFrame');
			if (widgetBox) {
				clearInterval(widgetReady);
				var parent;
				var element = e.target;
				//appleModalFix(element, widgetBox);
				var elRect = element.getBoundingClientRect();
				if (element.classList.contains('wdgtIcon')) {
					parent = findAncestor(element, 'class', 'widget');
				}
				else {
					parent = element;
				}
				...
				var modalBoxBB = widgetBox.getBoundingClientRect();
				var modalBoxWidth = modalBoxBB.width;
				widgetBox.width = parentWidth;
				modalBoxLeft = parentLeft;
				
				// left placement of the modal (modal has a shift to center)
				
				widgetBox.style.left = modalBoxLeft + 10 + 'px';
				widgetBox.style.width = parentWidth - 20 + 'px';
			}
		}, 500);
	}
}
