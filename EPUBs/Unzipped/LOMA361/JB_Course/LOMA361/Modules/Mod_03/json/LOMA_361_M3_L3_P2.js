Lesson_Data_File([
    {
        "Page_Setup": {
            "Assets_folder": "Mod",
            "Lesson_Header_Color": "#0070b0",
            "Lesson_Title_Color": "#ffffff",
            "Lesson_Title_Text": "",
            "Page_Title": "Comparative Analysis",
            "Progress_Bar_Color": "#f2c400",
            "Progress_Bar_Height": "3px",
            "heroSize": "75%",
            "hImage": "",
            "hImage_Alt_Tag_Text": ""
        },
        "JBuilder_Content": [
            {
                "Heading": [
                    {
                        "sngl_Heading": "Comparative Analysis"
                    },
                    {
                        "alignment": "center"
                    },
                    {
                        "top_margin": "0px"
                    },
                    {
                        "size": "h2"
                    },
                    {
                        "style": "bold"
                    },
                    {
                        "color": "#000000"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "border_color": ""
                    },
                    {
                        "border_weight": ""
                    },
                    {
                        "border_style": ""
                    },
                    {
                        "border_radius": "0px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "#FBF6D9",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "solid",
                        "border_radius": "0px",
                        "heading": "Learning Objective",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Use an index number and trend analysis to analyze trends"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "In the chapter on financial ratios, we showed how a ratio result is typically more useful when it is compared to something else, rather than looked at in isolation. Similarly, the numbers on an insurer’s financial statements are generally more useful when insurers engage in <em>comparative analysis</em>. In comparative analysis, an insurer can (1) analyze the change in an item over two or more accounting periods, in which case the analysis is known as <em>horizontal analysis</em>; or (2) compare two sets of numbers that reflect company operations during the same accounting period, in which case the analysis is known as <em>vertical analysis</em>. To achieve the most meaningful results, an insurer typically uses both forms of analysis in tandem."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Horizontal Analysis",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Horizontal analysis</span> is a type of comparative analysis that involves calculating the absolute monetary amount of change in a particular account value on a specified financial statement from one reporting period to another. Horizontal analysis uses <span class=\"keyterm\" tabindex=\"0\" role=\"button\">comparative financial statements</span>, which present data for two or more accounting periods side by side. Comparative financial statements can be formed from multiple balance sheets, income statements, cash flow statements, and statements of owners’ equity. Comparative financial statements can present year-to-year changes in terms of (1) absolute numerical amounts or (2) percentages or ratios."
                    },
                    {
                        "Paragraph_txt": "<br />Figure 14.1 shows excerpts from the comparative balance sheets and income statements of the Luna Life Insurance Company."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Figure 14.1 Comparative Financial Statements</strong>"
                    }
                ]
            },
            {
                "image_w_text": [
                    {
                        "image_name": "14-1figure.jpg",
                        "image_w_text_tag": "Balance Sheet ($000s omitted)<br /><br />[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th>&nbsp;</th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2017</p></th><th><p style=\"text-align: center;\">Amount Increase/Decrease</p></th><th><p style=\"text-align: center;\">Percent Increase/Decrease</p></th></tr><tr><th><p>Total Assets</p></th><td><p style=\"text-align: right;\">$11,825,060</p></td><td><p style=\"text-align: right;\">$13,901,278</p></td><td><p style=\"text-align: right;\">($2,076,218)</p></td><td><p style=\"text-align: right;\">(14.9%)</p></td></tr></tbody></table>[[NEEDS EDIT!!]]Income Statement ($000s omitted)<br /><br />[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th>&nbsp;</th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2017</p></th><th><p style=\"text-align: center;\">Amount Increase/Decrease</p></th><th><p style=\"text-align: center;\">Percent Increase/Decrease</p></th></tr><tr><th><p>Net Income</p></th><td><p style=\"text-align: right;\">$705,031</p></td><td><p style=\"text-align: right;\">$627,433</p></td><td><p style=\"text-align: right;\">$77,598</p></td><td><p style=\"text-align: right;\">12.4%</p></td></tr></tbody></table>[[NEEDS EDIT!!]]",
                        "border_style": "",
                        "border_height": "",
                        "border_color": ""
                    },
                    {
                        "image_position": "bottom",
                        "image_aligmnent": "center"
                    },
                    {
                        "image_size": "50%"
                    },
                    {
                        "image_text": "",
                        "image_list": [],
                        "list_position": "bottom",
                        "top_margin": "10px",
                        "Type_of_List": "ol",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1",
                        "list_border_style": "",
                        "list_border_height": "",
                        "list_border_color": "",
                        "image_text_below": ""
                    },
                    {
                        "image_text_color": "#bf0d3f",
                        "image_text_style": "normal",
                        "image_text_variant": "normal",
                        "image_text_weight": "normal",
                        "image_text_top_margin": "0%",
                        "image_text_padding": "0px"
                    },
                    {
                        "image_text_alignmnet": "left",
                        "image_font_size": "jb1"
                    },
                    {
                        "top_margin": "0px",
                        "snippet_top_margin": "10px"
                    },
                    {
                        "background_color": ""
                    },
                    {
                        "image_radius": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "The financial reporting period chosen as the reference point for horizontal analysis is known as the <span class=\"keyterm\" tabindex=\"0\" role=\"button\">base period</span>. A base period could be a specific month, quarter, year, or other appropriate period of time. In most cases, the base period is the earliest period. In the above example, the base period for Luna’s comparative balance sheet is 2017, the earlier financial reporting period. For truly accurate analysis, a company must be sure it is including identical categories for analysis in each period. A category that is present in one period and absent in another can weight the findings, which makes the analysis less accurate and useful."
                    },
                    {
                        "Paragraph_txt": "<br />To calculate the absolute monetary amount of the change being analyzed, subtract the base period amount from the amount of the period under study. To calculate the percentage change, divide the absolute monetary amount by the base period amount, and then multiply the result by 100, as follows:"
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Absolute monetary amount = </strong>"
                    },
                    {
                        "Paragraph_txt": "<br /><strong>Period under study amount – Base period amount</strong>"
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "[[Equation Table. NEEDS EDIT!]]"
                            }
                        ],
                        "Row_Heading_Color": "transparent",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#000000"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Percentage change"
                                    },
                                    {
                                        "Data_Info": "="
                                    },
                                    {
                                        "Data_Info": "<br> Absolute monetary amount"
                                    },
                                    {
                                        "Data_Info": "x 100"
                                    }
                                ],
                                "Row_Data_Color": "",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Base period amount"
                                    }
                                ],
                                "Row_Data_Color": "",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "<span style=\"font-size:14px;font-weight:bold\">EXAMPLE</span>",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Example.</strong> The Aries Life Insurance Company had total assets of $13,901,278 in 2017 and total assets of $14,825,060 in 2018. Because 2017 is the earlier period, it is the base period, and 2018 is the period under study. The absolute monetary amount of the change in assets is $923,782, which represents a 6.6% increase in total assets from 2017 to 2018. This increase in total assets is calculated as follows:"
                    },
                    {
                        "Paragraph_txt": "<br />($14,825,060 – $13,901,278) ÷ $13,901,278 = 0.066 x 100 = 6.6%"
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "Trend Analysis",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h3",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "A <span class=\"keyterm\" tabindex=\"0\" role=\"button\">trend</span> is a consistent pattern of change displayed over time. <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Trend analysis</span>, also called <em>trend percentages</em> or <em>index-number trend analysis</em>, is a specific type of horizontal analysis that involves calculating percentage changes in financial statement items over several successive accounting periods, rather than over just two periods. Comparative financial statements indicate the direction and velocity of trends:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "<em>Direction</em> refers to whether a trend displays an increase or decrease in an account."
                            },
                            {
                                "lstText": "<em>Velocity</em> refers to whether the increase or decrease in an account is gradual or rapid."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h1",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "To conduct trend analysis on financial statements, use the following procedure:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "Select a base period. Suppose you wanted to conduct a trend analysis on the years 2014–2018. Typically, the base period would be the earliest period in the series: 2014 would be the base year—that is, the year to which you would compare all other years. However, if you chose to compare all years to 2015, then 2015—not the earliest year of 2014—would be the base period. Note that the amounts in the base period must be positive amounts; a trend cannot be accurately analyzed if it has a negative amount in the base period."
                            },
                            {
                                "lstText": "Next, list all the amounts in the base period as 100. Each 100 represents 100 percent and is an index number. An <span class=\"keyterm\" tabindex=\"0\" role=\"button\">index number</span> is a statistical method for measuring the change in a variable."
                            },
                            {
                                "lstText": "Finally, express each corresponding amount from nonbase years—the periods under study—as a percentage of the applicable base year amount. To do so, first divide the nonbase value by the base value, and then multiply the result by 100. For example, if the base year of 2014 had an amount of $20,000 and the nonbase year of 2015 had an amount of $15,000, then you would record an index number of 75 ($15,000 ÷ $20,000 = 0.75, or 75%). Index numbers can be calculated only when the amounts are positive."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "List_text_paragraph": [
                    {
                        "List_Heading": "",
                        "heading_alignment": "center",
                        "heading_style": "bold",
                        "heading_underline": "",
                        "heading_underline_color": "",
                        "heading_underline_height": "0px",
                        "heading_color": "",
                        "heading_size": "h1",
                        "paragraph_alignment": "left",
                        "paragraph_text_size": "jb1",
                        "paragraph_top_text_color": "#000000",
                        "paragraph_bot_text_color": "#000000"
                    },
                    {
                        "Top_Paragraphs": [
                            {
                                "Paragraph_text": "Use the following rules to interpret index numbers:"
                            }
                        ]
                    },
                    {
                        "align_text": "left"
                    },
                    {
                        "Type_of_List": "ul",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1"
                    },
                    {
                        "top_margin": "10px"
                    },
                    {
                        "background_color": "#ffffff"
                    },
                    {
                        "padding": "10px"
                    },
                    {
                        "text_color": "#000000"
                    },
                    {
                        "list_items": [
                            {
                                "lstText": "An index number greater than 100 indicates an increase in the variable from the base period. For example, an index number of 120 indicates a 20 percent increase from the base period."
                            },
                            {
                                "lstText": "An index number less than 100 indicates a decrease in the variable from the base period. For example, an index number of 80 indicates a 20 percent decrease from the base period."
                            }
                        ]
                    },
                    {
                        "Bottom_paragraphs": []
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Note that in many cases, a company will conduct trend analysis using multiple base periods. For example, an insurer conducting a trend analysis on the years 2014–2018 might use 2014 as the base period for analyzing the 2015 amounts, use 2015 as the base period for analyzing the 2016 amounts, and so on. This approach helps analyze growth over time. Watch Video 14.1 for an example of trend analysis."
                    }
                ]
            },
            {
                "Video_Files_New": [
                    {
                        "Video": "https://loma.azureedge.net/videos/LOMA361/14-1video.mp4",
                        "Expert_View": "no",
                        "Expert_View_Alt": "The is for the expert view video series",
                        "top_margin": "20px",
                        "back_color": "#ffffff",
                        "Transcript_Text_font": "jb1",
                        "Transcript_Text_align": "left",
                        "Transcript_Text_color": "#000000",
                        "Person_font": "jb2",
                        "Person_align": "left",
                        "Person_text_color": "#000000",
                        "Person_underline": ""
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<strong>Trend Analysis Example</strong>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "A trend analysis is a fairly simple process of comparing index numbers. Let’s look at a trend analysis on the years 2015-2019."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<strong>Step One: Select the base period.</strong> In this case, the base period is 2015."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th colspan=\"6\"><p style=\"text-align: center;\">($000s)</p></th></tr><tr><th>&nbsp;</th><th><p style=\"text-align: center;\">2015</p></th><th><p style=\"text-align: center;\">2016</p></th><th><p style=\"text-align: center;\">2017</p></th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2019</p></th></tr><tr><th><p>Premium income</p></th><td><p style=\"text-align: right;\">$2,300</p></td><td><p style=\"text-align: right;\">$3,700</p></td><td><p style=\"text-align: right;\">$3,500</p></td><td><p style=\"text-align: right;\">$4,400</p></td><td><p style=\"text-align: right;\">$6,000</p></td></tr><tr><th><p>Benefit payments</p></th><td><p style=\"text-align: right;\">$1,467</p></td><td><p style=\"text-align: right;\">$2,295</p></td><td><p style=\"text-align: right;\">$2,201</p></td><td><p style=\"text-align: right;\">$3,018</p></td><td><p style=\"text-align: right;\">$4,020</p></td></tr></tbody></table>[[NEEDS EDIT!!]]"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "The “2015” column is highlighted and labeled “Base period.”"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<strong>Step Two: List all the amounts in the base period as 100.</strong> Remember, each 100 is an index number that represents 100%."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th colspan=\"6\"><p style=\"text-align: center;\">Index Numbers</p></th></tr><tr><th>&nbsp;</th><th><p style=\"text-align: center;\">2015</p></th><th><p style=\"text-align: center;\">2016</p></th><th><p style=\"text-align: center;\">2017</p></th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2019</p></th></tr><tr><th><p>Premium income</p></th><td><p style=\"text-align: right;\">100</p></td><td><p style=\"text-align: right;\">$3,700</p></td><td><p style=\"text-align: right;\">$3,500</p></td><td><p style=\"text-align: right;\">$4,400</p></td><td><p style=\"text-align: right;\">$6,000</p></td></tr><tr><th><p>Benefit payments</p></th><td><p style=\"text-align: right;\">100</p></td><td><p style=\"text-align: right;\">$2,295</p></td><td><p style=\"text-align: right;\">$2,201</p></td><td><p style=\"text-align: right;\">$3,018</p></td><td><p style=\"text-align: right;\">$4,020</p></td></tr></tbody></table>[[NEEDS EDIT!!]]"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<strong>Step Three: Express each corresponding amount from nonbase years as a percentage of the applicable base year amount.</strong>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "To do this, divide each nonbase value by the base value, and then multiply the result by 100."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "For example, the base period of 2015 has a premium income amount of $2,300 and the non-base period of 2016 has a premium income amount of $3,700. Therefore, the percentage of the base year amount is:"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "<strong>($3,700 ÷ $2,300) x 100 = 161</strong>"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th colspan=\"6\"><p style=\"text-align: center;\">Index Numbers</p></th></tr><tr><th>&nbsp;</th><th><p style=\"text-align: center;\">2015</p></th><th><p style=\"text-align: center;\">2016</p></th><th><p style=\"text-align: center;\">2017</p></th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2019</p></th></tr><tr><th><p>Premium income</p></th><td><p style=\"text-align: right;\">100</p></td><td><p style=\"text-align: right;\">161</p></td><td><p style=\"text-align: right;\">$3,500</p></td><td><p style=\"text-align: right;\">$4,400</p></td><td><p style=\"text-align: right;\">$6,000</p></td></tr><tr><th><p>Benefit payments</p></th><td><p style=\"text-align: right;\">100</p></td><td><p style=\"text-align: right;\">$2,295</p></td><td><p style=\"text-align: right;\">$2,201</p></td><td><p style=\"text-align: right;\">$3,018</p></td><td><p style=\"text-align: right;\">$4,020</p></td></tr></tbody></table>[[NEEDS EDIT!!]]"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "Repeat this process for each nonbase year amount."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th colspan=\"6\"><p style=\"text-align: center;\">Index Numbers</p></th></tr><tr><th>&nbsp;</th><th><p style=\"text-align: center;\">2015</p></th><th><p style=\"text-align: center;\">2016</p></th><th><p style=\"text-align: center;\">2017</p></th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2019</p></th></tr><tr><th><p>Premium income</p></th><td><p style=\"text-align: right;\">100</p></td><td><p style=\"text-align: right;\">161</p></td><td><p style=\"text-align: right;\">152</p></td><td><p style=\"text-align: right;\">191</p></td><td><p style=\"text-align: right;\">261</p></td></tr><tr><th><p>Benefit payments</p></th><td><p style=\"text-align: right;\">100</p></td><td><p style=\"text-align: right;\">156</p></td><td><p style=\"text-align: right;\">150</p></td><td><p style=\"text-align: right;\">206</p></td><td><p style=\"text-align: right;\">274</p></td></tr></tbody></table>[[NEEDS EDIT!!]]"
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "By comparing the index numbers, we can see that in 2016 and 2017, premium income increased more than its benefit payments did."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "However, in 2018 and 2019, benefit payments outpaced premium income."
                    },
                    {
                        "Person": "",
                        "Transcript_Text": "The company’s management would want to investigate the reasons behind this trend."
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "30px",
                        "image": "Lets Review.jpg",
                        "image_alt_text": "",
                        "question": "The Luna Life Insurance Company is using horizontal analysis to compare revenues from two separate accounting periods. The revenues in the base period were $24,700,000, and the revenues in the period under study were $26,600,000. What is the percentage value of the change between the two periods?",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "7.1%",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "7.7%",
                                "true_false": true,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "92.8%",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "107.7%",
                                "true_false": false,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "The percentage value is equal to the absolute monetary amount ÷ the base period amount, in this case calculated as ($26,600,000 – $24,700,000) ÷ $24,700,000 = .07692, rounded to 7.7%.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "The percentage value is equal to the absolute monetary amount ÷ the base period amount, in this case calculated as ($26,600,000 – $24,700,000) ÷ $24,700,000 = .07692, rounded to 7.7%.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "#FBF6D9",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "0px",
                        "image": "",
                        "image_alt_text": "",
                        "question": "Suppose you are conducting a trend analysis. The value for your base period is $6,100, and the value for the following period is $7,015. What are the index numbers for these two values?",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "Base period 87 / Following period 100",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Base period 100 / Following period 87",
                                "true_false": false,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Base period 100 / Following period 115",
                                "true_false": true,
                                "q_feedback": ""
                            },
                            {
                                "q_answer": "Base period 115 / Following period 100",
                                "true_false": false,
                                "q_feedback": ""
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "The index number for the base period is always 100. The index number for periods under study is a percentage of the base number. The formula for calculating the percentage is (period under study ÷ base period) x 100. In this example, the formula is (7,015 ÷ 6,100) x 100 = 115.",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "The index number for the base period is always 100. The index number for periods under study is a percentage of the base number. The formula for calculating the percentage is (period under study ÷ base period) x 100. In this example, the formula is (7,015 ÷ 6,100) x 100 = 115.",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Vertical Analysis",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<span class=\"keyterm\" tabindex=\"0\" role=\"button\">Vertical analysis</span> is a type of comparative analysis that reveals the relationship of each financial statement item to a specified financial statement item during the same reporting period. Vertical analysis provides additional context for the figures on a financial statement, thus making it easier for managers to determine if an amount represents a particular strength of the company or if it indicates a potential cause for concern. Figure 14.2 compares vertical analysis and horizontal analysis."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Figure 14.2 Horizontal and Vertical Analysis</strong>"
                    }
                ]
            },
            {
                "image_w_text": [
                    {
                        "image_name": "14-2figure.jpg",
                        "image_w_text_tag": "Figure: A section of a balance sheet.<br /><br />In the balance sheet, the Total Assets amount from 2018 and the Total Assets amount from 2017 are highlighted in red.<br /><br />Text in red: Horizontal analysis compares the amount of a particular item from one reporting period to another.<br /><br />In the balance sheet, the Debt Securities percent from 2018 and the Total Securities Available percent from 2018 are highlighted in purple.<br /><br />Text in purple: Vertical analysis compares the relationship between two items in the same accounting period.",
                        "border_style": "",
                        "border_height": "",
                        "border_color": ""
                    },
                    {
                        "image_position": "bottom",
                        "image_aligmnent": "center"
                    },
                    {
                        "image_size": "50%"
                    },
                    {
                        "image_text": "",
                        "image_list": [],
                        "list_position": "bottom",
                        "top_margin": "10px",
                        "Type_of_List": "ol",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1",
                        "list_border_style": "",
                        "list_border_height": "",
                        "list_border_color": "",
                        "image_text_below": ""
                    },
                    {
                        "image_text_color": "#bf0d3f",
                        "image_text_style": "normal",
                        "image_text_variant": "normal",
                        "image_text_weight": "normal",
                        "image_text_top_margin": "0%",
                        "image_text_padding": "0px"
                    },
                    {
                        "image_text_alignmnet": "left",
                        "image_font_size": "jb1"
                    },
                    {
                        "top_margin": "0px",
                        "snippet_top_margin": "10px"
                    },
                    {
                        "background_color": ""
                    },
                    {
                        "image_radius": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Vertical analysis can be applied to many financial statements. Most commonly, insurers conduct vertical analysis on the balance sheet and the income statement, as summarized in the following table."
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Balance Sheet"
                            },
                            {
                                "Heading": "Income Statement"
                            }
                        ],
                        "Row_Heading_Color": "#000000",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Divide asset item by total assets"
                                    },
                                    {
                                        "Data_Info": "Divide revenue item by total revenues"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Divide liabilities and owners’ equity item by total liabilities and owners’ equity"
                                    },
                                    {
                                        "Data_Info": "Divide expense item by total revenues or by total expenses, depending on the purpose of the vertical analysis"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Vertical analysis on the balance sheet and income statement allows management to detect potential problems across the company."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "<span style=\"font-size:14px;font-weight:bold\">EXAMPLE</span>",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Example.</strong> Murcielago Life Insurance runs a vertical analysis to compare last year’s premium income to last year’s total revenues. The analysis reveals that premium income made up 57% of Murcielago’s total revenues last year. This percentage is higher than Murcielago’s management usually anticipates for premium income."
                    },
                    {
                        "Paragraph_txt": "<br /><strong>Analysis.</strong> Murcielago’s management can easily see that premium income played a larger role in the company’s success last year than anticipated. Murcielago’s next action would probably be to use horizontal analysis to investigate whether premium income was much higher than anticipated last year, or whether its invested assets had lower income than anticipated, thus leaving premium income to contribute a greater overall percentage of the company’s total revenues."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "We can also use vertical analysis to compile a <span class=\"keyterm\" tabindex=\"0\" role=\"button\">common-size financial statement</span>, which is a financial statement that includes the percentages obtained from vertical analysis. Common-size financial statements often display only percentages; monetary amounts for each item may not be given. The balance sheet and the income statement are often exhibited as common-size statements."
                    },
                    {
                        "Paragraph_txt": "<br />Each item on a common-size balance sheet is expressed as a percentage of (1) total assets or (2) total liabilities and owners’ equity. In other words, the amount of a company’s total assets is the “common size” to which we reference the balance sheet’s other asset amounts. Similarly, total liabilities and owners’ equity is the “common size” to which we reference the balance sheet’s liability amounts and owners’ equity amounts."
                    },
                    {
                        "Paragraph_txt": "<br />Most commonly, insurers use common-size financial statements to compare their company with other companies in the industry. Common-size balance sheets allow insurers to compare financial strength and liquidity, among other qualities. Similarly, an insurer can use common-size income statements to compare profitability, expense control, and other measures. Figure 14.3 shows a simplified example of a common-size balance sheet."
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Figure 14.3 Simplified Common-Size Balance Sheet</strong>"
                    }
                ]
            },
            {
                "image_w_text": [
                    {
                        "image_name": "14-3figure.jpg",
                        "image_w_text_tag": "Chart: Puma Life Insurance Company Common-Size Balance Sheet, December 31, 2018<br /><br />[[NEEDS EDIT!!]]<table class=\"blue striped cellborders\"><tbody><tr><th><p>Assets</p></th><th><p style=\"text-align: center;\">2018</p></th><th><p style=\"text-align: center;\">2017</p></th></tr><tr><td><p>Total securities available for sale</p></td><td><p style=\"text-align: right;\">80.7%</p></td><td><p style=\"text-align: right;\">85.7%</p></td></tr><tr><td><p>Other assets</p></td><td><p style=\"text-align: right;\">19.3%</p></td><td><p style=\"text-align: right;\">11.3%</p></td></tr><tr><td><p><strong>Total Assets</strong></p></td><td><p style=\"text-align: right;\">100%</p></td><td><p style=\"text-align: right;\">100%</p></td></tr><tr><th colspan=\"3\"><p>Liabilities and Stockholders<strong>’</strong>&nbsp;Equity</p></th></tr><tr><td><p>Total policy liabilities</p></td><td><p style=\"text-align: right;\">73.0%</p></td><td><p style=\"text-align: right;\">70.5%</p></td></tr><tr><td><p>Other liabilities</p></td><td><p style=\"text-align: right;\">3.9%</p></td><td><p style=\"text-align: right;\">4.9%</p></td></tr><tr><td><p><strong>Total Liabilities</strong></p></td><td><p style=\"text-align: right;\">76.9%</p></td><td><p style=\"text-align: right;\">75.4%</p></td></tr><tr><td><p><strong>Total Stockholders’ Equity</strong></p></td><td><p style=\"text-align: right;\">23.1%</p></td><td><p style=\"text-align: right;\">24.6%</p></td></tr><tr><td><p><strong>Total Liabilities and Stockholders’ Equity</strong></p></td><td><p style=\"text-align: right;\">100%</p></td><td><p style=\"text-align: right;\">100%</p></td></tr></tbody></table>[[NEEDS EDIT!!]]",
                        "border_style": "",
                        "border_height": "",
                        "border_color": ""
                    },
                    {
                        "image_position": "bottom",
                        "image_aligmnent": "center"
                    },
                    {
                        "image_size": "50%"
                    },
                    {
                        "image_text": "",
                        "image_list": [],
                        "list_position": "bottom",
                        "top_margin": "10px",
                        "Type_of_List": "ol",
                        "ul_type": "",
                        "ol_type": "",
                        "List_Text_Size": "jb1",
                        "list_border_style": "",
                        "list_border_height": "",
                        "list_border_color": "",
                        "image_text_below": ""
                    },
                    {
                        "image_text_color": "#bf0d3f",
                        "image_text_style": "normal",
                        "image_text_variant": "normal",
                        "image_text_weight": "normal",
                        "image_text_top_margin": "0%",
                        "image_text_padding": "0px"
                    },
                    {
                        "image_text_alignmnet": "left",
                        "image_font_size": "jb1"
                    },
                    {
                        "top_margin": "0px",
                        "snippet_top_margin": "10px"
                    },
                    {
                        "background_color": ""
                    },
                    {
                        "image_radius": ""
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "",
                        "heading_font_size": "h2",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Because amounts on common-size financial statements are stated in relative terms rather than in absolute terms, insurers can use common-size statements to more accurately compare companies of different sizes in the same industry."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "<span style=\"font-size:14px;font-weight:bold\">EXAMPLE</span>",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Example.</strong> For 2018, the Immensity Insurance Company’s net income of $580,000,000 is greater than the Humility Life Insurance Company’s net income of $110,000,000. As net profits are determined after expenses are subtracted, you might assume from this information that Immensity is the more profitable company. However, let’s examine the simplified common-size income statements for both companies, presented below."
                    },
                    {
                        "Paragraph_txt": "<br /><strong>Analysis.</strong> Here we see that Humility’s net income represents a greater percentage of its total revenues (14%) than does Immensity’s net income (11%). Therefore, although Humility’s net income is lower in amount, Humility appears to be operating more efficiently than is Immensity."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "JB_Table": [
                    {
                        "Col_Heading": [
                            {
                                "Heading": "Common-Size Income Statements"
                            },
                            {
                                "Heading": "Immensity"
                            },
                            {
                                "Heading": "Humility"
                            }
                        ],
                        "Row_Heading_Color": "#000000",
                        "Row_Heading_Height": "",
                        "Row_Heading_Font_Whole": "center,#ffffff"
                    },
                    {
                        "Row_Information": [
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "<strong>Revenues</strong>"
                                    },
                                    {
                                        "Data_Info": "&nbsp;"
                                    },
                                    {
                                        "Data_Info": "&nbsp;"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Premiums"
                                    },
                                    {
                                        "Data_Info": "53%"
                                    },
                                    {
                                        "Data_Info": "55%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Net investment income"
                                    },
                                    {
                                        "Data_Info": "22%"
                                    },
                                    {
                                        "Data_Info": "23%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Fee income"
                                    },
                                    {
                                        "Data_Info": "25%"
                                    },
                                    {
                                        "Data_Info": "22%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Total Revenues"
                                    },
                                    {
                                        "Data_Info": "100%"
                                    },
                                    {
                                        "Data_Info": "100%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "<strong>Expenses</strong>"
                                    },
                                    {
                                        "Data_Info": "&nbsp;"
                                    },
                                    {
                                        "Data_Info": "&nbsp;"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Benefits and claims"
                                    },
                                    {
                                        "Data_Info": "47%"
                                    },
                                    {
                                        "Data_Info": "49%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Operating expenses"
                                    },
                                    {
                                        "Data_Info": "36%"
                                    },
                                    {
                                        "Data_Info": "31%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Taxes"
                                    },
                                    {
                                        "Data_Info": "6%"
                                    },
                                    {
                                        "Data_Info": "6%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Total Expenses"
                                    },
                                    {
                                        "Data_Info": "89%"
                                    },
                                    {
                                        "Data_Info": "86%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .09)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            },
                            {
                                "Row_Data": [
                                    {
                                        "Data_Info": "Net Income"
                                    },
                                    {
                                        "Data_Info": "11%"
                                    },
                                    {
                                        "Data_Info": "14%"
                                    }
                                ],
                                "Row_Data_Color": "rgba(0, 0, 0, .3)",
                                "Row_Data_Height": "",
                                "Row_Data_Font_Whole": "center,#000000"
                            }
                        ]
                    },
                    {
                        "Whole_Table_Settings": [
                            {
                                "Top_Margin": "10px",
                                "Heading_Text_Settings": "",
                                "Row_Text_Settings": "",
                                "Table_Background_Color": "",
                                "Table_Border_Settings": "block",
                                "Table_Width": "100%",
                                "Table_Max_Height": ""
                            }
                        ]
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "Benchmarking",
                        "heading_font_size": "h3",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "Common-size financial statements are useful for benchmarking purposes. <span class=\"keyterm\" tabindex=\"0\" role=\"button\">Benchmarking</span>, or <em>running a benchmark</em>, is a process that consists of (1) identifying the best outcomes that other companies have achieved for a specific activity or process and the practices that produced those outcomes, and (2) implementing those practices to equal or surpass the best outcomes. Insurers may benchmark for both financial and nonfinancial practices; here, we focus on financial benchmarking."
                    },
                    {
                        "Paragraph_txt": "<br />To conduct expense analysis using benchmarking, an insurer first determines a set of selection criteria, such as company size or region of operations, and selects a group of appropriate companies according to these criteria. The insurer then acquires data regarding these companies’ expense experience by line of business, function, or another classification."
                    },
                    {
                        "Paragraph_txt": "<br />Where possible, insurers commonly benchmark against other insurance companies, other financial services organizations—such as banks and mutual fund companies—and even companies in other industries. An insurer may also run a comparison between the company’s own operations and expenses and the average results of the entire industry. Because the industry average contains data from companies that may be much larger or smaller than the insurer running the benchmark, the analysis may indicate larger variations than the insurer might have received from benchmarking against companies of similar size. However, an industry-wide comparison can indicate whether a company is exceeding or falling behind in any particular area. An insurer might also have a larger or smaller competitor in a specific market."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Paragraphs_New": [
                    {
                        "Paragraph_align": "left",
                        "Paragraph_Font_Size": "jb1",
                        "Paragraph_Font_color": "",
                        "Paragraph_Font_style": "",
                        "Paragraph_Font_variant": "",
                        "Paragraph_Font_weight": "",
                        "Padding": "",
                        "background_color": "",
                        "top_margin": "20px",
                        "border_color": "black",
                        "border_weight": "thin",
                        "border_style": "",
                        "border_radius": "0px",
                        "heading": "<span style=\"font-size:14px;font-weight:bold\">EXAMPLE</span>",
                        "heading_font_size": "h4",
                        "heading_alignment": "center",
                        "heading_color": "",
                        "heading_weight": "bold",
                        "heading_style": "normal",
                        "heading_variant": "normal",
                        "heading_transform": "",
                        "columns": "1",
                        "line_spacing": "1.5",
                        "background_image": ""
                    },
                    {
                        "Paragraph_txt": "<strong>Example.</strong> The Trio Life Insurance Company decides to run a benchmark to compare its expenses with the expenses of other companies in the industry. Trio Life gathers publicly released financial statements from three companies of similar size—Alpha Life, Beta Financial, and Gamma Life—and prepares common-size financial statements based on these statements. Trio Life discovers that both Alpha Life and Gamma Life have lower total expenses as a percentage of total revenues on their income statements, compared to Trio Life. Trio Life’s management decides to research further what cost control measures these companies are using, in hopes that Trio Life might benefit from similar measures."
                    }
                ]
            },
            {
                "Divider": [
                    {
                        "divider_image": ""
                    },
                    {
                        "height": "50px",
                        "width": "60%"
                    },
                    {
                        "top_margin": "50px"
                    },
                    {
                        "flip": "false"
                    },
                    {
                        "back_color": ""
                    },
                    {
                        "Line_Display": "true",
                        "Line_Style": "solid",
                        "Line_height": "2px",
                        "Line_Color": "#000000",
                        "top_margin": "50px"
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "30px",
                        "image": "Lets Review.jpg",
                        "image_alt_text": "",
                        "question": "Trend analysis is a practice associated with (<strong>horizontal analysis</strong> / <strong>vertical analysis</strong>.)",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "horizontal analysis",
                                "true_false": true,
                                "q_feedback": "Trend analysis is a specific type of horizontal analysis that involves calculating percentage changes in financial statement items over several successive accounting periods, rather than over just two periods."
                            },
                            {
                                "q_answer": "vertical analysis",
                                "true_false": false,
                                "q_feedback": "Trend analysis is a specific type of <em>horizontal analysis</em> that involves calculating percentage changes in financial statement items over several successive accounting periods, rather than over just two periods."
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            },
            {
                "Question_Comp": [
                    {
                        "Question_type": "1",
                        "text_color": "#000000",
                        "question_size": "jb1",
                        "background_color": "#FBF6D9",
                        "border_color": "",
                        "border_weight": "",
                        "border_style": "",
                        "border_radius": "10px",
                        "question_align": "left",
                        "feedback_align": "left",
                        "top_margin": "0px",
                        "image": "",
                        "image_alt_text": "",
                        "question": "Insurers can use (<strong>horizontal analysis</strong> / <strong>vertical analysis</strong>) to compile a common-size financial statement.",
                        "question_image": "",
                        "question_image_placement": "none",
                        "question_image_alt_tag": ""
                    },
                    {
                        "paragraph_top_text": []
                    },
                    {
                        "qList": [
                            {
                                "qList_on_off": "off",
                                "align_text": "left",
                                "Type_of_List": "ol",
                                "ul_type": "",
                                "ol_type": "",
                                "Heading_Text_Size": "h1"
                            }
                        ]
                    },
                    {
                        "paragraph_bottom_text": [
                            {
                                "paragraph_bottom": ""
                            }
                        ]
                    },
                    {
                        "question_answers": [
                            {
                                "q_answer": "horizontal analysis",
                                "true_false": false,
                                "q_feedback": "A company uses <em>vertical analysis</em> to compile a common-size financial statement."
                            },
                            {
                                "q_answer": "vertical analysis",
                                "true_false": true,
                                "q_feedback": "Common-size financial statements are useful for benchmarking purposes."
                            }
                        ]
                    },
                    {
                        "correct_feedback": "That's correct!",
                        "correct_remediation": "",
                        "incorrect_feedback": "Sorry. That's incorrect.",
                        "incorrect_remediation": "",
                        "correct_incorrect_font_size": "jb1",
                        "Additional_feedback": "",
                        "Additional_font_size": "jb1"
                    }
                ]
            }
        ]
    }
]);