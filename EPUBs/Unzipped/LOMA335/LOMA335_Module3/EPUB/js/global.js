...
function ResponsiveCellHeaders(table) {
	...
	for (var h = 0; h < headerRows.length; h++) {
		...
		for (i = 0; i < ths.length; i++) {
			...
			headingText = headingText.replace(/<p [^>]+>/g, "");
			headingText = headingText.replace(/<p>/g, "");
			headingText = headingText.replace(/<\/p>/g, "");
			headingText = headingText.replace(/<br\s?\/?>/g, "");
...
function AddTableARIA(tblContainer) {
    ...
    for (i = 0; i < allRows.length; i++) {
        ...
        else if (!allRows[i].querySelector('td') && !allRows[i].classList.contains('hide-header')) {
